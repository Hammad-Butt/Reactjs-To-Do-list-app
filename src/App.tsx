...
function App() {
  ...
  useEffect(() => {
    document.title = 'To-Do List App'; // Set the page title
  }, []);
...
function TaskInput({ currentTask, setCurrentTask, handleAddTask, isEditing }: TaskInputProps) {
  return (
...
interface TaskItemProps {
  ...
  isEditing: boolean;  // Add this prop to access the editing state
}

function TaskItem({ task, handleDeleteTask, handleEditTask, handleToggleComplete, isEditing }: TaskItemProps) {
  return (
    <li className={`task-item ${task.completed ? 'completed' : ''}`}>
      <span onClick={() => handleToggleComplete(task.id)}>{task.name}</span>
      <div className="task-actions">
        <button
          className={`edit ${isEditing ? 'active' : ''}`}
